...
def neighbor(point):
    ...
    for origin in Total_points:
        if distance(origin,point) <= 3:
            count += 1
    return count
map.fillcontinents(color = '#DEDBBC', lake_color = '#83C1DA')
map.drawmapboundary(fill_color = '#83C1DA')
...
startcolor = '#57ABCD'   #白色，读者可以自行修改
endcolor = '#89250C'          #蓝色，读者可以自行修改
cmap2 = col.LinearSegmentedColormap.from_list('own2',['#83C1DA','#FF4500'])
# extra arguments are N=256, gamma=1.0
cm.register_cmap(cmap=cmap2)
cm1 = cm.get_cmap('own2')
cm2 = plt.cm.hot
map.contourf(X,Y,data,10,alpha =0.8,cmap = cm1,zorder = 999999999)
