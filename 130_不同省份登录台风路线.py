...
def draw_province_name():
    label_latlon = {
        'Guangdong ' : (113.07-0.5,23.10 - 0.3),
        'Fujian' : (118.22-2,27.04-1.5),
        'Zhejiang': (120.51 - 1.5,29.09-0.8),
        'Shanghai':(121.23+0.5,31.04)
    }
...
map = Basemap(projection='cyl',lat_0=90,lon_0=120,\
            llcrnrlat=15 ,urcrnrlat=25,\
            llcrnrlon=105,urcrnrlon=115,\
            rsphere=6371200.,resolution='h',area_thresh=1000)
...
count = 0
province = get_denglu_province()
for i in range(len(dengluid)):
    print('描绘第{}个台风中/{}'.format(i+1,250))
    ID = dengluid[i]
    print(ID)
    path = pathes[ID]
    path_type = dengluty[i]
    pathcolor = 'black'
    if ID not in province or province[ID] != '广东':
        continue
    for j in range(len(path) - 1 ):
        ...
        ax1.arrow(x1, y1,x2 - x1,y2 - y1,
             ...
             head_width=0.01, head_length=0.01, fc= pathcolor, ec=pathcolor,linewidth = 0.1)
        #print()
